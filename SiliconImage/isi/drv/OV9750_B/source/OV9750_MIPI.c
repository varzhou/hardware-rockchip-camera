...
#define  Sensor_NEWEST_TUNING_XML "OV9750_v1.0"
 ...
 *****************************************************************************/
CREATE_TRACER( Sensor_INFO , "OV9750: ", INFO,    0U );
CREATE_TRACER( Sensor_WARN , "OV9750: ", WARNING, 1U );
CREATE_TRACER( Sensor_ERROR, "OV9750: ", ERROR,   1U );

CREATE_TRACER( Sensor_DEBUG, "OV9750: ", INFO,     0U );

CREATE_TRACER( Sensor_REG_INFO , "OV9750: ", INFO, 0);
CREATE_TRACER( Sensor_REG_DEBUG, "OV9750: ", INFO, 0U );

#define Sensor_SLAVE_ADDR       0x6cU                           /**< i2c slave address of the OV9750 camera sensor */
#define Sensor_SLAVE_ADDR2      0x20U
#define Sensor_SLAVE_AF_ADDR    0x18U                           /**< i2c slave address of the OV9750 integrated AD5820 */
#define Sensor_OTP_SLAVE_ADDR   0x36U
 ...
 *****************************************************************************/
const char Sensor_g_acName[] = "OV9750_MIPI";

extern const IsiRegDescription_t Sensor_g_aRegDescription[];
extern const IsiRegDescription_t Sensor_g_1280x960[];
extern const IsiRegDescription_t Sensor_g_1280x960_30fps[];
extern const IsiRegDescription_t Sensor_g_1280x960_20fps[];
extern const IsiRegDescription_t Sensor_g_1280x960_15fps[];
extern const IsiRegDescription_t Sensor_g_1280x960_10fps[];
    ...
    else
    {
        if(mipi_lanes == SUPPORT_MIPI_TWO_LANE){
            switch (pIsiSensorCaps->Index) 
            {
                ...
                case 1:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_1280_960P20;
                    break;
                }
                case 2:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_1280_960P15;
                    break;
                }

                case 3:
                {
                    pIsiSensorCaps->Resolution = ISI_RES_1280_960P10;
                    break;
                }
                default:
                {
			...
			case ISI_RES_1280_960P10:
	        {
	            if (set2Sensor == BOOL_TRUE) {                    
                    if (res_no_chg == BOOL_FALSE) {
						if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_1280x960)) != RET_SUCCESS){
							result = RET_FAILURE;
							TRACE( Sensor_ERROR, "%s: failed to set two lane Sensor_g_1280x960 \n", __FUNCTION__ );
			            }
					}
                    

        	if (pConfig->Resolution == ISI_RES_1280_960P30) {						 
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1280x960_30fps);
					}else if (pConfig->Resolution == ISI_RES_1280_960P20) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1280x960_20fps);
					}else if (pConfig->Resolution == ISI_RES_1280_960P15) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1280x960_15fps);
					}else if (pConfig->Resolution == ISI_RES_1280_960P10) {
						result = IsiRegDefaultsApply( (IsiSensorHandle_t)pSensorCtx, Sensor_g_1280x960_10fps);
					}  
                }
	         	

	            usLineLengthPck = 0x032a;
				pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 800;
				
				if (pConfig->Resolution == ISI_RES_1280_960P30) {
	            	usFrameLengthLines = 0x07b8;					
					TRACE( Sensor_INFO, "%s: ISI_RES_1280_960P30 \n", __FUNCTION__ );
				}else if(pConfig->Resolution == ISI_RES_1280_960P20) {
	            	usFrameLengthLines = 0x0b92; 
					TRACE( Sensor_INFO, "%s: ISI_RES_1280_960P20 \n", __FUNCTION__ );
				}else if(pConfig->Resolution == ISI_RES_1280_960P15) {				
					TRACE( Sensor_INFO, "%s: ISI_RES_1280_960P15 \n", __FUNCTION__ );
	            	usFrameLengthLines = 0x0f6e; 
				}else if(pConfig->Resolution == ISI_RES_1280_960P10) {
					TRACE( Sensor_INFO, "%s: ISI_RES_1280_960P10 \n", __FUNCTION__ );
	            	usFrameLengthLines = 0x1724; 
				}
	            break;
...
)
{
    ...
    if(1)
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3508, (usGain>>8)&0xff);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
...
)
{
   ...
   if ( NewIntegrationTime > pSensorCtx->AecMaxIntegrationTime ) NewIntegrationTime = pSensorCtx->AecMaxIntegrationTime;
    ...
    {
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3500, (CoarseIntegrationTime >> 12) & 0x0f );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3501, (CoarseIntegrationTime >> 4) & 0xff );
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
        result = Sensor_IsiRegWriteIss( pSensorCtx, 0x3502, (CoarseIntegrationTime & 0x0f) << 4);
        RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
   ...
   // TRACE( Sensor_ERROR, "%s: SetTi=%f NewTi=%f\n", __FUNCTION__, *pSetIntegrationTime,NewIntegrationTime);
	...
	TRACE( Sensor_DEBUG, "%s:\n"
         ...
         *pSetIntegrationTime,NewIntegrationTime,CoarseIntegrationTime,
         result_intertime,
         (CoarseIntegrationTime >> 12) & 0x0f ,
         (CoarseIntegrationTime >> 4) & 0xff,
         (CoarseIntegrationTime & 0x0f) << 4);
    return ( result );
}
			...
			case SUPPORT_MIPI_TWO_LANE:
			{
				switch(Resolution)
				{
					...
					case ISI_RES_1280_960P10:
						...
						AFPSCHECKANDADD( ISI_RES_1280_960P20);
						AFPSCHECKANDADD( ISI_RES_1280_960P15);
						AFPSCHECKANDADD( ISI_RES_1280_960P10);
						break;
				}
