...
GraphConfig::getSensorFrameParams(rk_aiq_frame_params &sensorFrameParams)
{
    ...
    LOGI("%s: PixelArray output: w: %d, h: %d, crop l: %d, crop t: %d",
    ...
    if (ret != css_err_none) {
        ...
        LOGI("%s: scaler output  w: %d, h: %d, crop w: %d, crop h: %d",
                __FUNCTION__,
                wScaler, hScaler,
                lScaler, tScaler);
    }
    ...
    LOGI("------------------- sensorFrameParams ---------------------------");
    LOGI("%s: Final cropped Image w = %d, Final cropped Image h = %d",
                 __FUNCTION__, wCroppedImage, hCroppedImage);

    LOGI("%s: Horizontal_crop_offset = %d, Vertical_crop_offset = %d",
